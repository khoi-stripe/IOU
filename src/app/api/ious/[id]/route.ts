...
export async function PATCH(
  ...
  try {
    ...
    if (action === "repaid") {
      ...
      // Only the person who owes (from_user) can mark it as repaid
      if (iou.from_user_id !== userId) {
        return NextResponse.json(
          { error: "Only the person who owes can mark this as repaid" },
          { status: 403 }
        );
      }

      const updated = await markIOURepaid(id);
      if (!updated) {
        return NextResponse.json(
